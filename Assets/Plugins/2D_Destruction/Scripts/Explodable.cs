...
public class Explodable : MonoBehaviour
{
    ...
    {
        // Only explode if hit by "Big Guy" and hasn't already exploded
        if (!hasExploded && collision.gameObject.CompareTag("Big Guy"))
        {
            hasExploded = true;
            explode();
            // ensure destruction in case explode() path didn't destroy immediately
            if (this != null)
            {
                Destroy(gameObject);
            }
    ...
    {
        // handle trigger-based collisions as well
        if (!hasExploded && other.gameObject.CompareTag("Big Guy"))
        {
            hasExploded = true;
            explode();
            if (this != null)
    ...
    public void explode()
    {
        ...
        // Immediately hide original to avoid visible remnants
        ...
        // destroy the original object after activating fragments (always)
        Destroy(gameObject);
    }
    /// <summary>
